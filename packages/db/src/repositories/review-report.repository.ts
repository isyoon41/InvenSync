...
export class ReviewReportRepository implements IReviewReportRepository {
  async create(input: CreateReviewReportInput): Promise<ReviewReport> {
    return prisma.reviewReport.create({
      ...
      include: { evidences: { orderBy: { sortOrder: 'asc' }, include: { searchResult: { select: { id: true, markName: true, applicationNumber: true, registerNumber: true, applicantName: true, classNo: true, statusLabel: true, relevanceScore: true, designatedGoodsSummary: true } } } }, inquiry: { select: { proposedMarkName: true, title: true } } },
